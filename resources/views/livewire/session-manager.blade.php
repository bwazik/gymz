<div>
    <div class="flex items-center justify-between mb-5">
        <h2 class="text-xl font-bold text-white tracking-tight">⚡ Active Sessions</h2>
    </div>
        ...
        <div
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                @if ($session->status === \App\Enums\SessionStatus::Scheduled)
                    <span
                        class="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-500/15 text-blue-400 border border-blue-500/30">
                        Scheduled
                    </span>
            ...
            @if ($session->status === \App\Enums\SessionStatus::Scheduled)
                @if ($isRoleA)
                    ...
                    <div class="bg-white/5 border border-white/10 rounded-2xl p-6 text-center">
                        <p class="text-xs text-white/50 mb-4 font-medium">Show this QR to your workout partner</p>
                        <div class="inline-block bg-white rounded-2xl p-4 mb-4">
                            {!! QrCode::size(150)->style('round')->color(15, 23, 42)->generate($session->qr_token) !!}
                        </div>
                        <div class="mt-3">
                            <p class="text-[10px] text-white/30 mb-1">Or share this token manually:</p>
                            <code
                                class="text-xs bg-white/10 px-3 py-1.5 rounded-lg text-gymz-accent font-mono select-all border border-white/10">
                                {{ $session->qr_token }}
                            </code>
                        </div>
                    </div>
                @else
                    ...
                    <div class="bg-white/5 border border-white/10 rounded-2xl p-5">
                        <p class="text-xs text-white/50 mb-3 font-medium">Enter the token from your partner's QR code
                        </p>
                        <div class="flex gap-2">
                            <input type="text" wire:model="scannedToken" placeholder="Paste token here..."
                                class="flex-1 rounded-xl bg-white/5 border border-white/10 text-white text-sm px-4 py-2.5 focus:border-gymz-accent/50 focus:ring-1 focus:ring-gymz-accent/30 transition-colors placeholder-white/20 font-mono">
                            <button wire:click="verifyToken({{ $session->id }})" wire:loading.attr="disabled"
                                class="px-4 py-2.5 rounded-xl bg-gymz-accent text-gymz-dark font-semibold text-sm hover:bg-gymz-accent/90 transition-all disabled:opacity-50 shadow-lg shadow-gymz-accent/25">
            ...
            @elseif ($session->status === \App\Enums\SessionStatus::InProgress)
                ...
                <div class="bg-gymz-accent/5 border border-gymz-accent/20 rounded-2xl p-5 text-center">
                    ...
                    <p class="text-xs text-white/50 mb-4">When you're done, end the session to claim your Glutes reward.
                    </p>
                    <button wire:click="endSession({{ $session->id }})" wire:loading.attr="disabled"
                        ...
                        class="w-full py-3 rounded-xl bg-gradient-to-r from-gymz-accent to-emerald-600 text-gymz-dark font-bold text-sm hover:opacity-90 transition-opacity shadow-lg shadow-gymz-accent/25 disabled:opacity-50">
    ...
    @empty
        ...
        <div class="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8 text-center shadow-glass">
            ...
            <h3 class="text-white/70 font-medium mb-1">No active sessions</h3>
            <p class="text-sm text-white/40">Once a workout request is accepted, your session will appear here with a QR
