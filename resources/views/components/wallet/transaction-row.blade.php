...
@php
    ...
    $amountClass = $isPositive ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400';
    $amountPrefix = $isPositive ? '+' : '';
    $iconBg = $isPositive ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500';
@endphp
...
<div
    ...
    <div class="text-left shrink-0">
        <p class="text-base font-black {{ $amountClass }} flex items-center justify-end gap-1">
            <span dir="ltr">{{ $amountPrefix }}{{ $transaction->amount }}</span>
        </p>
    </div>
</div>
